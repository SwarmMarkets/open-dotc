...
contract DotcV2 is ReentrancyGuardUpgradeable, ERC1155HolderUpgradeable, ERC721HolderUpgradeable {
     ...
     * @param specialAddress Special addresses involved in the trade, if any.
    ...
    function makeOffer(
        ...
        require(offer.expiryTimestamp > offer.timelockPeriod, "Dotc: Timelock must be less than expiration time");
        require(offer.expiryTimestamp > block.timestamp, "Dotc: Expiration timestamp has already expired");
        require(offer.timelockPeriod == 0 || offer.timelockPeriod > block.timestamp, "Dotc: Timelock period has already expired");
    ...
    function takeOffer(
        ...
        for (uint256 i = 0; i < offer.specialAddress.length; ) {
            if (offer.specialAddress[i] != address(0)) {
                require(offer.specialAddress[i] == msg.sender, "Dotc: You are not a designated buyer");
            }
    ...
    function _createOffer(
        ...
        _offer.isFullType = offer.isFullType;
        _offer.depositAsset = depositAsset;
        _offer.withdrawalAsset = withdrawalAsset;
        _offer.specialAddress = offer.specialAddress;
        _offer.expiryTime = offer.expiryTimestamp;
        _offer.timelockPeriod = offer.timelockPeriod;

        if (bytes(offer.terms).length != 0) _offer.terms = offer.terms;
